...
board_t * makeBoard(int w, int h, int numMines) {
  ...
  board->board = malloc(board->height * sizeof(*board->board));
  for (int i = 0; i < board->height; i++) {
    *(board->board + i) = malloc(board->width * sizeof(**board->board));
  }
  //initialize each square to be UNKNOWN
  for (int y = 0; y < board->height; y++) {
    for (int x = 0; x < board->width; x++) {
      board->board[y][x] = UNKNOWN;
    }
  }
  for (int n = 0; n < board->totalMines; n++) {
    addRandomMine(board);
  }
  return board;
}
