...
//to make c-string pointed by *strp starting with not space
...
//decide whether c is one of +-*/ (operators)
...
Expression * parse(const char ** strp) {  //parameter: pointer to c-string
  skipSpace(strp);
  if (**strp == '\0') {
    ...
    *strp = *strp + 1;
    ...
    //number
    char * endp;
    long num = strtol(*strp, &endp, 10);  //endp store the first invalid character
    if (endp == *strp) {
      std::cerr << "Expected a number, but found " << *strp << "\n";
      return NULL;
    }
    *strp = endp;
    return new NumExpression(num);  //write a NumExpression
  }
}
